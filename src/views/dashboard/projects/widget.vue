            ...
            <span
              ...
              :class="{
                'bg-primary-subtle text-primary':
                  item.value.toUpperCase() === 'primary' || 'DOCUMENTOS PROCESADOS',
                'bg-warning-subtle text-warning':
                  item.value.toUpperCase() === 'PETICIONES POR RESPONDER',
                'bg-info-subtle text-info': item.value.toUpperCase() === 'info',
              }"
            >
              <template v-if="item.value.toUpperCase() == 'PETICIONES RESPONDIDAS'">
                <SendIcon size="24" class="text-primary"></SendIcon>
              </template>

              <template v-if="item.value.toUpperCase() == 'PETICIONES RECIBIDAS'">
                <CornerDownRightIcon size="24" class="text-primary"></CornerDownRightIcon>
              </template>

              <template v-if="item.value.toUpperCase() == 'PETICIONES POR RESPONDER'">
                <ClockIcon size="24" class="text-primary"></ClockIcon>
              </template>

              <template v-if="item.value.toUpperCase() == 'DOCUMENTOS PROCESADOS'">
